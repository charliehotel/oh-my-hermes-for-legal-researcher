...
## 📌 찾을 수 있는 것 (What You Can Find)

### ✅ Federal Regulations (연방 규칙)
**Federal Register** (연방 관보) 검색 — 미국 연방 정부의 공식 일간지입니다.

| 대상 | 범위 | 방법 |
|------|------|------|
| **Proposed rules** (입법예고) | 기관이 공개 의견을 받기 위해 제안한 규칙 | Federal Register API → 구조화된 Regulation Digest |
| **Final rules** (최종 규칙) | 발효일이 포함된 채택된 규칙 | 동일 |
| **Notices** (고시) | 기관 고시, 동의 명령, 정보 수집 안내 | 동일 |
| **NPRMs** (규칙 제정 제안 공고) | 의견 제출 마감일이 있는 공식 제안 | 동일 |

**대상 기관:** FTC, SEC, CFPB, FCC, EPA, DOJ, DOL, HHS 및 숫자 ID 조회로 200+ 기관.

### ✅ Case Law (판례)
웹 검색을 통한 미국 연방 및 주 법원 의견 검색.

| 유형 | 범위 | 한계 |
|------|------|------|
| **Federal cases** (연방 판례) | 대법원, 항소법원, 지방법원 | 인용에 `[verify]` 태그 — 항시 교차 검증 필요 |
| **State cases** (주 판례) | 공개된 주 항소심 의견 | 모든 주가 동일하게 커버되지는 않음 |
| **주요 주제 영역** | AI·저작권, 오픈소스·영업비밀, 프라이버시, 독점금지 | 질문하면 찾아드림 |

> 🔌 **선택 사항:** [CourtListener API](#courtlistener-api--verified-citations)(무료)를 연결하면 `[verify]` 대신 `[CourtListener — verified]` 태그 사용 가능.

### ✅ Statutes (법률)
웹 검색을 통한 미국 법전, 공법, 법안 검색.

| 유형 | 범위 | 한계 |
|------|------|------|
| **U.S. Code** (USC) | 조항별 연방 법률 | 웹 검색 전용 — Congress.gov 키 없이 전용 API 없음 |
| **Public Laws** | 최근 제정된 법률 | govinfo.gov 경유 |
| **Bills** | 계류 중인 법안 | Congress.gov 경유 |

### ✅ Secondary Sources (학술 자료)
웹 검색을 통한 법률 리뷰 논문, 법률 해설, 규제 분석 검색.

---

### ❌ 찾을 수 없는 것 (Limitations)

| 대상 | 사유 |
|------|------|
| **Non-U.S. law** (한국·EU·일본 법률) | 설계상 미국법만 대상 |
| **유료 데이터베이스** (Westlaw, LexisNexis) | 유료 구독 없음 — 무료/오픈 소스만 사용 |
| **주별 개별 법률** | 웹 검색으로 찾을 수는 있지만 구조화된 API 없음 |
| **최신 미판결 사건** | 공개 데이터베이스에 등록되는 데 시간 소요 |
| **법률 자문 또는 사건 전략** | 면책조항: 모든 출력은 변호사 검토가 필요한 초안 |
