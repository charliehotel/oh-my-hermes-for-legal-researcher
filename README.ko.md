...
| 대상 | 범위 | 방법 |
|------|------|------|
| **입법예고** | 기관이 공개 의견을 받기 위해 제안한 규칙 | Federal Register API → 구조화된 Regulation Digest |
| **최종 규칙** | 발효일이 포함된 채택된 규칙 | 동일 |
| **고시** | 기관 고시, 동의 명령, 정보 수집 안내 | 동일 |
| **NPRMs** | 의견 제출 마감일이 있는 공식 제안 | 동일 |
...
| 유형 | 범위 | 한계 |
|------|------|------|
| **연방 판례** | 대법원, 항소법원, 지방법원 | 인용에 `[verify]` 태그 — 항시 교차 검증 필요 |
| **주 판례** | 공개된 주 항소심 의견 | 모든 주가 동일하게 커버되지는 않음 |
